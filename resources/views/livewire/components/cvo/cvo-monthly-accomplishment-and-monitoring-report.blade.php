        ...
        <div class="modal-dialog modal-lg modal-dialog-scrollable modal-dialog-centered">
            <div class="modal-content">
                ...
                <div class="modal-body">
                    <div class="p-2" wire:loading.class="pe-none opacity-50">
                        ...
                        <!-- Submitted By -->
                        <div class="mb-10">
                            <label class="form-label required">Submitted By:</label>
                            <select class="form-select" wire:model.live="submitted_by">
                                <option value="">-Select-</option>
                                @foreach ($this->loadSignatories() as $item)
                        ...
                        <div class="mb-10">
                            <label class="form-label required">Recommending Approval:</label>
                            <select class="form-select" wire:model.live="recommending_approval">
                                <option value="">-Select-</option>
                                @foreach ($this->loadSignatories() as $item)
                                <option value="{{ $item->id }}">{{ $item->name }}</option>
                                @endforeach
                        ...
                        <div class="mb-10">
                            ...
                            <select class="form-select" wire:model.live="approved_by">
                                <option value="">-Select-</option>
                                @foreach ($this->loadSignatories() as $item)
                                <option value="{{ $item->id }}">{{ $item->name }}</option>
                                @endforeach
                ...
                <div class="modal-footer">
                    ...
                    <div wire:loading.remove>
                        <button type="submit" class="btn btn-primary" wire:click="generateMonthlyAccomplishmentAndMonitoringReportPdf">Generate</button>
                    </div>
                    <div wire:loading wire:target="generateMonthlyAccomplishmentAndMonitoringReportPdf">
...
<script>
    ...
    $wire.on('show-monthly-accomplishment-and-monitoring-report-modal', () => {
        if ($wire.selectedAccomplishmentMonth) {
            $('#generateMonthlyAccomplishmentAndMonitoringReportPdfModal').modal('show');
        } else {
            $wire.generateMonthlyAccomplishmentAndMonitoringReportPdf();
        }
    });
</script>
@endscript