<div>
                ...
                <div class="card card-xxl-stretch" wire:loading.class="opacity-50 pe-none">
                    ...
                    <div class="card-header border-0 py-5">
                        <h3 class="card-title align-items-start flex-column">
                            <span class="card-label fw-bolder fs-3 mb-1">Document Type</span>
                            <span class="text-muted fw-bold fs-7">Over {{ $document_types->count() }} types</span>
                        </h3>
                        <div class="card-toolbar">
                            @can('reference.incomingRequestCategory.create')
                            <a href="#" class="btn btn-icon btn-secondary" data-bs-toggle="modal" data-bs-target="#documentTypeModal"><i class="bi bi-plus-circle"></i></a>
                            @endcan
                        </div>
                    </div>
                    <!--end::Header-->
                    ...
                    <div class="card-body d-flex flex-column" style="position: relative;">
                        ...
                        <div class="table-responsive" wire:loading.class="opacity-50" wire:target.except="saveIncomingRequestCategory">
                                    ...
                                    <tr>
                                        ...
                                        <td>
                                            <div class="btn-group" role="group">
                                                ...
                                                <button type="button" class="btn btn-icon btn-sm" title="Edit PDF" style="background-color:#c01f1f;border-color:#fd7e14;color:#fff;" wire:click="openPdfEditor({{ $item->id }})">
                                                ...
                                                <button
                                                    ...
                                                    <div wire:loading.remove wire:target="{{ $item->deleted_at ? 'restoreIncomingRequestCategory' : 'deleteIncomingRequestCategory' }}({{ $item->id }})">
                                                        <i class="bi {{ $item->deleted_at ? 'bi-arrow-counterclockwise' : 'bi-trash' }}"></i>
                                                    </div>
                                                    <div wire:loading wire:target="{{ $item->deleted_at ? 'restoreIncomingRequestCategory' : 'deleteIncomingRequestCategory' }}({{ $item->id }})">
                                                        <div class="spinner-border spinner-border-sm" role="status"><span class="visually-hidden">Loading...</span></div>
                                                    </div>
                                                </button>
                                                @endcan
                        ...
                        <!--begin::Pagination-->
                        <div class="pt-3">
                            {{ $document_types->links(data: ['scrollTo' => false]) }}
                        </div>
                        <!--end::Pagination-->

                        <div class="resize-triggers">
                            <div class="expand-trigger"><div style="width: 404px; height: 426px;"></div></div>
    ...
    <!--begin::Modal - Add / Edit Document Type-->
    <div class="modal fade" tabindex="-1" id="documentTypeModal" data-bs-backdrop="static" data-bs-keyboard="false" wire:ignore.self>
        <div class="modal-dialog modal-dialog-scrollable modal-dialog-centered">
            <div class="modal-content">
                <div class="modal-header">
                    <h5 class="modal-title">{{ $editMode ? 'Edit' : 'Add' }} Document Type</h5>
    ...
    <!--end::Modal - Add / Edit Document Type-->

    <!--begin::Modal - Document Type Settings-->
    <div class="modal fade" tabindex="-1" id="docTypeSettingsModal" data-bs-backdrop="static" data-bs-keyboard="false" wire:ignore.self>
        <div class="modal-dialog modal-lg modal-dialog-scrollable modal-dialog-centered">
            <div class="modal-content">
                <div class="modal-header">
                    <h5 class="modal-title d-flex align-items-center gap-2">
                        <i class="bi bi-gear text-info fs-4"></i>
                        Settings &mdash; <span class="text-muted fw-normal">{{ $settingsDocTypeName }}</span>
                    </h5>
                ...
                <div class="modal-body">
                    ...
                    <div class="mb-8">
                        ...
                        <p class="text-muted fs-7 mb-4">Upload a letterhead or logo image that will appear at the top of the generated PDF for this document type. Recommended: PNG/JPG, max 2 MB.</p>
                        ...
                        <div>
                            ...
                            <input type="file" id="dtHeaderImageInput" class="form-control" accept="image/*">
                            @error('pdf_header_image')
                            <span class="text-danger fs-7 mt-1 d-block">{{ $message }}</span>
                            @enderror

                            <div id="dtNewHeaderPreview" class="mt-3 p-3 border border-dashed rounded bg-light-primary d-none">
                                <p class="fw-semibold fs-7 text-primary mb-2">Preview:</p>
                                <img id="dtNewHeaderPreviewImg" src="" alt="New header preview"
                                     class="rounded border"
                                     style="max-width: 100%; display: block;">
                            </div>
                        </div>
                    ...
                    <div>
                        ...
                        <p class="text-muted fs-7 mb-4">Define the signatories whose names and titles will appear at the bottom of the PDF for this document type.</p>

                        <div class="d-flex align-items-center gap-4 mb-4 p-3 bg-light rounded">
                            <label class="form-label fw-semibold mb-0 text-nowrap">Font Size</label>
                            <input type="range" class="form-range flex-grow-1"
                                   wire:model.live="signatory_font_size"
                                   min="8" max="24" step="1">
                            <span class="badge badge-light-primary fs-7 fw-bold" style="min-width:42px;text-align:center;">
                                {{ $signatory_font_size }}px
                            </span>
                        </div>
                        ...
                        <div class="table-responsive">
                            <table class="table table-bordered align-middle gs-3 gy-3">
                                ...
                                <tbody>
                                    @forelse ($docTypeSignatories as $i => $sig)
                                    <tr>
                                        <td>
                                            <input type="text"
                                                   class="form-control form-control-sm @error('docTypeSignatories.'.$i.'.name') is-invalid @enderror"
                                                   wire:model="docTypeSignatories.{{ $i }}.name"
                                                   placeholder="e.g. Juan Dela Cruz">
                                            @error('docTypeSignatories.'.$i.'.name')
                                            <span class="invalid-feedback">{{ $message }}</span>
                                            @enderror
                                        </td>
                                        <td>
                                            <input type="text"
                                                   class="form-control form-control-sm @error('docTypeSignatories.'.$i.'.title') is-invalid @enderror"
                                                   wire:model="docTypeSignatories.{{ $i }}.title"
                                                   placeholder="e.g. City Agriculturist">
                                            @error('docTypeSignatories.'.$i.'.title')
                                            <span class="invalid-feedback">{{ $message }}</span>
                                            @enderror
                                        </td>
                ...
                <div class="modal-footer">
                    ...
                    <button type="button" id="dtSaveSettingsBtn" class="btn btn-primary">
    ...
    <!--end::Modal - Document Type Settings-->

    <!--begin::Modal - PDF Editor-->
    <div class="modal fade" tabindex="-1" id="docTypePdfEditorModal" data-bs-backdrop="static" data-bs-keyboard="false" wire:ignore.self>
        <div class="modal-dialog modal-fullscreen">
            <div class="modal-content">
                <div class="modal-header">
                    <h5 class="modal-title d-flex align-items-center gap-2">
                        <i class="bi bi-file-earmark-pdf text-warning fs-4"></i>
                        Edit PDF Template &mdash; <span class="text-muted fw-normal" id="dtPdfEditorCategoryLabel">{{ $pdfEditorDocTypeName }}</span>
                    </h5>
                ...
                <div class="modal-body p-0 d-flex" style="overflow: hidden; height: calc(100vh - 130px);">
                    ...
                    <div class="d-flex flex-column flex-grow-1" style="min-width: 0;">
                        <div class="px-4 py-2 bg-light border-bottom d-flex align-items-center justify-content-between">
                            ...
                            <button type="button" class="btn btn-sm btn-light-success" id="dtPrintPreviewBtn">
                                <i class="bi bi-printer me-1"></i> Print Preview
                            </button>
                        </div>
                        <textarea
                            id="dtPdfTemplateEditor"
                    ...
                    <div style="width: 280px; min-width: 280px; overflow-y: auto;" class="bg-light">
                        ...
                        <div class="p-3">
                            ...
                            <div class="d-flex align-items-center justify-content-between mb-2">
                                ...
                                <button
                                    type="button"
                                    class="btn btn-xs btn-light font-monospace fs-8 py-1 px-2 dt-copy-var-btn"
                                    data-var="{{ '{{' . $var . '}' . '}' }}"
    ...
    <script>
        ...
        $wire.on('show-doc-type-settings-modal', () => {
            $('#docTypeSettingsModal').modal('show');
        });

        $wire.on('hide-doc-type-settings-modal', () => {
            $('#docTypeSettingsModal').modal('hide');
            _dtPendingHeaderFile = null;
            const preview = document.getElementById('dtNewHeaderPreview');
            const img     = document.getElementById('dtNewHeaderPreviewImg');
            if (preview) preview.classList.add('d-none');
            if (img) img.src = '';
        });

        $wire.on('show-doc-type-pdf-editor-modal', () => {
            const label = document.getElementById('dtPdfEditorCategoryLabel');
            if (label) label.textContent = $wire.pdfEditorDocTypeName;
            $('#docTypePdfEditorModal').modal('show');
        });

        $wire.on('hide-doc-type-pdf-editor-modal', () => {
            $('#docTypePdfEditorModal').modal('hide');
        });

        // ── Header image preview (deferred upload) ────────────────────────
        let _dtPendingHeaderFile = null;

        document.addEventListener('change', function (e) {
            if (e.target.id !== 'dtHeaderImageInput') return;
            const file = e.target.files[0];
            if (!file || !file.type.startsWith('image/')) { _dtPendingHeaderFile = null; return; }
            _dtPendingHeaderFile = file;
        ...
        document.addEventListener('click', function (e) {
            if (!e.target.closest('#dtSaveSettingsBtn')) return;
            const btn = document.getElementById('dtSaveSettingsBtn');
            btn.disabled = true;
            ...
            if (_dtPendingHeaderFile) {
                $wire.upload('pdf_header_image', _dtPendingHeaderFile,
                    () => { _dtPendingHeaderFile = null; $wire.saveSettings(); finish(); },
                    ()  => { finish(); }
                );
            } else {
                $wire.saveSettings();
        ...
        document.addEventListener('click', function (e) {
            const btn = e.target.closest('.dt-copy-var-btn');
            if (!btn) return;
            const varText = btn.dataset.var;
            const editor  = document.getElementById('dtPdfTemplateEditor');

            if (editor && document.activeElement === editor) {
                const start = editor.selectionStart;
                const end   = editor.selectionEnd;
                editor.value = editor.value.slice(0, start) + varText + editor.value.slice(end);
        ...
        // ── Shared: build rendered HTML ───────────────────────────────────
        function buildDtPdfPreviewHtml() {
            const editor = document.getElementById('dtPdfTemplateEditor');
            ...
            const sigFontSize = $wire.pdfEditorSignatoryFontSize || 12;
            let signatoriesHtml = '';
            if (sigs.length > 0) {
                ...
                const cells = sigs.map(s =>
                    ...
                    `<strong style="font-size:${sigFontSize}px;display:block;">${s.name}</strong>` +
                    `<span style="font-size:${sigFontSize - 1}px;color:#555;">${s.title}</span>` +
                    `</div></div>`
                ).join('');
            ...
            const sampleData = {
                ...
                category_no:                  'Sample Category-001',
                ...
                document_type_name:           $wire.pdfEditorDocTypeName,
        ...
        // ── Print Preview ─────────────────────────────────────────────────
        document.getElementById('dtPrintPreviewBtn')?.addEventListener('click', function () {
            let html = buildDtPdfPreviewHtml();
            if (!html) return;
